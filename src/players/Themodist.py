...
class Themodist(BasePlayer):
    def __init__(self, confpath, midiobj):
        ...
        with open(confpath, encoding="utf-8") as f:
            conf = json.load(f)
        self.accomp_vacuum = conf["expression"]["base"]
        self.theme_vacuum = conf["expression"]["accent"]

    def emulate_expression(self, curtime):
        self.bass_vacuum = self.treble_vacuum = self.accomp_vacuum
        if self.holes["bass_snakebite"]["is_open"]:
            self.bass_vacuum = self.theme_vacuum

        if self.holes["treble_snakebite"]["is_open"]:
            self.treble_vacuum = self.theme_vacuum
